...
class FormContractor implements FormContractorInterface
{
    ...
    {
        $formOptions                             = [];
        $formOptions['sonata_field_description'] = $fieldDescription;

        switch ($type) {
            // case TreeModelType::class:
            case 'doctrine_phpcr_odm_tree':
                $formOptions['class']         = $fieldDescription->getTargetModel();
                $formOptions['model_manager'] = $fieldDescription->getAdmin()->getModelManager();

                break;
            case ModelType::class:
            case 'sonata_type_model':
            case ModelListType::class:
            case 'sonata_type_model_list':
                if ('child' !== $fieldDescription->getMappingType() && !$fieldDescription->getTargetModel()) {
            ...
            case 'sonata_type_collection':
                ...
                $formOptions['type']         = AdminType::class;
                $formOptions['modifiable']   = true;
    ...
    protected function getAssociationAdminException(FieldDescriptionInterface $fieldDescription): \LogicException
    {
        ...
        if (\in_array($fieldDescription->getMappingType(), [ClassMetadata::MANY_TO_ONE, ClassMetadata::MANY_TO_MANY, 'referrers'], true)) {
            if ($fieldDescription->getTargetModel()) {
                $msg .= " for the target document: `{$fieldDescription->getTargetModel()}`";
            }
